...
// src/lsp_ext.ts
var lc = __toESM(require("coc.nvim"));
...
var shuffleCrateGraph = new lc.RequestType0("rust-analyzer/shuffleCrateGraph");
...
var viewFileText = new lc.RequestType("rust-analyzer/viewFileText");
...
var inlayHints = new lc.RequestType("textDocument/inlayHint");
...
function createClient(bin, config) {
  let folder = ".";
  if (import_coc.workspace.workspaceFolders.length) {
    folder = import_coc.Uri.parse(import_coc.workspace.workspaceFolders[0].uri).fsPath;
  }
  ...
  if (import_coc.workspace.workspaceFolders.length && standalone(import_coc.workspace.workspaceFolders[0].uri)) {
    const docs = import_coc.workspace.documents.filter((doc) => isRustDocument(doc.textDocument));
    if (docs.length) {
      initializationOptions = { detachedFiles: docs.map((doc) => import_coc.Uri.parse(doc.uri).fsPath), ...initializationOptions };
    }
  }
    ...
    middleware: {
      async provideHover(document2, position, token) {
        ...
        if (mode === "v" || mode === "V") {
          ...
          positionOrRange = await import_coc.window.getSelectedRange(mode);
        }
        if (!positionOrRange)
...
var Config = class {
  ...
  async onConfigChange(event) {
    ...
    if (!reload) {
      ...
      reload = await import_coc2.window.showPrompt(`${msg}. Reload now?`);
    }
    if (reload) {
      await import_coc2.commands.executeCommand(`rust-analyzer.reload`);
    }
  }
  get serverPath() {
    var _a;
  ...
  get restartServerOnConfigChange() {
    return this.cfg.get("restartServerOnConfigChange");
  }
  get inlayHints() {
  ...
  get prompt() {
    return this.cfg.get("updates.prompt", true);
  }
  get channel() {
...
var HintsUpdater = class {
  ...
  async activate() {
    ...
    import_coc4.workspace.onDidChangeTextDocument((e) => {
      ...
      if (doc && isRustDocument(doc.textDocument)) {
        if (import_coc4.events.insertMode && !this.ctx.config.inlayHints.refreshOnInsertMode) {
          return;
        }
        this.syncAndRenderHints(doc);
      }
  ...
  async renderHints(doc, hints) {
    ...
    if (this.ctx.config.inlayHints.typeHints) {
      ...
      for (const item of decorations.type) {
        ...
        if (label.startsWith(": "))
          label = label.replace(": ", "");
        const chunks = [[`${sep}${label}`, "CocRustTypeHint"]];
        if (chaining_hints[item.position.line] === void 0) {
    ...
    if (this.ctx.config.inlayHints.chainingHints) {
      ...
      for (const item of decorations.chaining) {
        ...
        if (!label.length)
          continue;
        const chunks = [[`${sep}${label}`, "CocRustChainingHint"]];
        if (chaining_hints[item.position.line] === void 0) {
          chaining_hints[item.position.line] = chunks;
        } else {
          chaining_hints[item.position.line].push(split);
  ...
  async fetchHints(file) {
    ...
    const range = import_coc4.Range.create(0, 0, file.doc.lineCount - 1, endChar);
...
var Ctx2 = class {
    ...
    client.onNotification(serverStatus, async (status) => {
      ...
      if (status.health !== "ok" && ((_a = status.message) == null ? void 0 : _a.length)) {
        if (status.message.startsWith("cargo check failed"))
          return;
        import_coc5.window.showNotification({ content: status.message });
        import_coc5.window.showWarningMessage(`rust-analyzer failed to start, run ':CocCommand rust-analyzer.reloadWorkspace' to reload`);
      }
    });
    this.client = client;
  }
  ...
  async checkUpdate(auto = true) {
    ...
    if (old === latest.tag) {
      if (!auto) {
        import_coc5.window.showInformationMessage(`Your Rust Analyzer release is updated`);
      }
      return;
    }
    ...
    if (ret === 0) {
      ...
      try {
        ...
        import_coc5.window.showInformationMessage(msg2, "error");
        return;
      }
      await this.client.stop();
      this.client.start();
...
function joinLines2(ctx) {
  return async () => {
    ...
    if (mode)
      range = await import_coc6.window.getSelectedRange(mode);
    if (!range) {
...
function ssr2(ctx) {
  return async () => {
    ...
    const mode = await import_coc6.workspace.nvim.call("visualmode");
    if (mode) {
      const range = await import_coc6.window.getSelectedRange(mode);
      if (range)
...
function serverVersion(ctx) {
  return async () => {
    ...
    if (!bin) {
      ...
      import_coc6.window.showErrorMessage(msg);
      return;
    }
...
async function fetchRunnable(ctx) {
  ...
  if (!isRustDocument(document2))
    return;
  import_coc6.window.showInformationMessage(`Fetching runnable...`);
  const params = {
    textDocument: { uri: document2.uri },
    position
  };
...
function runSingle(ctx) {
  return async (runnable) => {
    ...
    terminal = await import_coc6.window.createTerminal(opt);
...
function toggleInlayHints(ctx) {
  return async () => {
    if (!ctx.config.inlayHints.chainingHints) {
      import_coc6.window.showWarningMessage(`Inlay hints for method chains is disabled. Toggle action does nothing;`);
      return;
    }
...
async function applySnippetWorkspaceEdit(edit) {
  ...
  if (import_vscode_languageserver_protocol2.TextDocumentEdit.is(change)) {
    const newEdits = [];
    for (const indel of change.edits) {
      ...
      newEdits.push(import_coc6.TextEdit.replace(range, newText));
    }
    ...
    if (selection) {
      await import_coc6.window.selectRange(selection);
    } else if (position) {
...
function resolveCodeAction(ctx) {
  return async (params) => {
    var _a;
    params.command = void 0;
    const item = await ctx.client.sendRequest(import_vscode_languageserver_protocol2.CodeActionResolveRequest.method, params);
...
function viewFileText2(ctx) {
  return async () => {
    const { document: document2 } = await import_coc6.workspace.getCurrentState();
    if (!isRustDocument(document2))
      return;
    const ret = await ctx.client.sendRequest(viewFileText, { uri: document2.uri });
    if (!ret)
      return;
    const nvim = import_coc6.workspace.nvim;
    nvim.pauseNotification();
...
function echoRunCommandLine(ctx) {
  return async () => {
    ...
    import_coc6.window.showInformationMessage(commandLine);
  };
}
...
function moveItemUp(ctx) {
  return moveItem2(ctx, "Up" /* Up */);
}
function moveItemDown(ctx) {
  return moveItem2(ctx, "Down" /* Down */);
}
...
function shuffleCrateGraph2(ctx) {
  return async () => {
    const { document: document2 } = await import_coc6.workspace.getCurrentState();
    if (!isRustDocument(document2))
      return;
    await ctx.client.sendRequest(shuffleCrateGraph);
  };
}
...
async function activate(context) {
  ...
  ctx.registerCommand("viewFileText", viewFileText2);
  ...
  ctx.registerCommand("shuffleCrateGraph", shuffleCrateGraph2);
  ...
  ctx.registerCommand("reload", (ctx2) => {
    return async () => {
      import_coc7.window.showInformationMessage(`Reloading rust-analyzer...`);
      ...
      import_coc7.window.showInformationMessage(`Reloaded rust-analyzer`);
    };
  });
